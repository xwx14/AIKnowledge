...
采集数据保存到 `knowledge/raw/` 目录，文件名格式：`{主题}-{日期}.json`

同日期同主题重复运行时，按序号递增避免覆盖：
- 第 1 次：`github-trending-2026-04-16.json`
- 第 2 次：`github-trending-2026-04-16-2.json`
- 第 3 次：`github-trending-2026-04-16-3.json`

命名前须先用 Glob 检查目标目录是否已有同名文件，若存在则自动追加序号。
